...
public class FileEditorServlet extends HttpServlet {
    ...
    /**
     * Shows a JavaScript confirm box to warn the user from unsaved changes by leaving the FileEditorServlet.
     *
     * @param  out  a PrintWriter to create HTML code in this method
     */
    private void unsavedChangesWarning(final PrintWriter out) {
    ...
    protected void processRequest(final HttpServletRequest request, final HttpServletResponse response)
            ...
            if ((textToSave != null) && !cancel.equals("true")) {
                try {
                    if (file.exists() || file.canRead()) {
                        final FileWriter fw = new FileWriter(file.getCanonicalFile());
                        final BufferedWriter f = new BufferedWriter(fw);
                        f.write(textToSave);
                        f.close();
                    }
                } catch (Exception e) {
        ...
        out.println("<HTML>\n"
                    ...
                    + "<br><font size=\"+1\" face=\"Courier New\"><b>" + fileName + " auf " + serverIP
                    + "</b></font><br><br>\n"
                    + "<form name=\"editorFormular\" action=\"http://" + serverIP + ":" + httpPort
                    + "/fe/fileeditor\" method=\"post\">\n"
