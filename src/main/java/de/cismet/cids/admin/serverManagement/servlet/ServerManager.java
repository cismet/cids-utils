...
public class ServerManager extends HttpServlet {
    ...
    private final static String VALIDATION_ATTRIBUTE = "validation_attribute";
    private final static String LOGIN_PARAMETER_NAME = "login";
    private final static String LOGIN_PARAMETER_VALUE = "validate";
    private final static String LOGOUT_PARAMETER_VALUE = "logout";
     ...
     */
    private void editServerConfigFile(PrintWriter out) {
        out.println("<script type=\"text/javascript\">\n" +
                "<!--\n" +
                "window.open(\"http://" + serverIP + ":" + httpPort + "/fileeditor\", \"EditorWindow" + httpPort + "\", \"width=750,height=550,left=25,top=25,resizable=yes\");\n" + //, location=yes, menubar=yes
                "//-->\n" +
                "</script>\n");
    }
    
    private void getServerLogFile(PrintWriter out) {
        out.println("<script type=\"text/javascript\">\n" +
                "<!--\n" +
                "window.open(\"http://" + serverIP + ":" + httpPort + "/serverlogfile\", \"Serverlog" + httpPort + "\", \"width=750,height=550,left=25,top=25,scrollbars=yes,resizable=yes\");\n" +
                "//-->\n" +
                "</script>\n");
    }
    ...
    private void refreshLogMessages() {
        ...
        try {
            if ( !logfile.exists() ) {
                logfile = new File(serverCon.getlogOutputDirectory() + File.separator + "logOutput" + httpPort + ".htm");
            }
            logfile.deleteOnExit();
            FileWriter f = new FileWriter(logfile);
            
            if (autoRefresh) {
                f.write("<html><head><title>LogOutput</title><meta http-equiv=\"pragma\" content=\"no-cache\"><meta http-equiv=\"cache-control\" content=\"no-cache\"></head><body text=\"#000000\" bgcolor=\"#FFFFFF\"><pre>" + logMessages + "</pre><a name=\"anchor\"></a></body></html>");
                // meta tags "pragma" and "cache-control" avert the Browser from caching the site
            } else {
                f.write("<html><head><title>LogOutput</title><meta http-equiv=\"pragma\" content=\"no-cache\"><meta http-equiv=\"cache-control\" content=\"no-cache\"></head><body text=\"#606060\" bgcolor=\"#FFFFFF\"><pre>" + logMessages + "</pre><a name=\"anchor\"></a></body></html>");
                // meta tags "pragma" and "cache-control" avert the Browser from caching the site
            }
            
            f.close();
        } catch (Exception e) {
            logger.error("Fehler beim Erstellen der oder beim Schreiben in die Datei logOutput" + httpPort +".htm!", e);
        }
    }
    ...
    /*private void killServer(PrintWriter out) {
        if (serverCon.isServerRunning()) {
            out.println("<script type=\"text/javascript\">\n" +
                        "<!--\n" +
                        "Check = confirm(\"Der Serverprozess l&auml;uft noch! Wenn Sie das Servermanagement jetzt beenden, wird der Server evtl. nicht ordnungsgem&auml;&szlig; heruntergefahren.\\nDiese Managementkonsole wird auch beendet werden!\\nWollen Sie das Servermanagement trotzdem beenden?\");\n" +
                        "if (Check == true) {\n" +
    ...
    /**
     * Depending on the given serverTask
     * the corresponding method will be invoked.
     * @param serverType ActionString indicating what to do
     */
    private void dispatchAction(String servertask) {
    ...
    /**
     * Checks wether a logout is in progress. This is done by
     * checking the parameter <b>login<b>. If the parameter has the
     * value <code>logout</code> a logout is in progress.
     * @return true if a logout is in progress
     */
    private boolean isLogoutInProgress(HttpServletRequest request) {
        boolean ret = false;
        
        String loginParam = request.getParameter( LOGIN_PARAMETER_NAME );
     ...
     */
    private void performLogout(HttpServletRequest request, PrintWriter out) {
        request.getSession(false).invalidate();
        
        out.println("Sie wurden erfolgreich ausgeloggt.");
    }
    
    private void printServerManager(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        /*if (servertask != null && servertask.equals("kill-now")) {
            logger.debug("killServerNow() aufgerufen.");
            killServerNow();
        }*/
        ...
        /*if (servertask != null && servertask.equals("kill")) {
            logger.debug("killServer(out) aufgerufen.");
            killServer(out);
        }*/
    ...
    private void printHeader(PrintWriter out) {
        ...
        //if (autoRefresh) {
        //out.println("<meta http-equiv=\"refresh\" content=\"5; URL=http://" + serverIP + ":" + httpPort + "/" + "cidsservermanager\">\n"); }
        out.println("<script type=\"text/javascript\">\n" +
                "<!--\n" +
                "function Refresh() {\n" +
    ...
    private void printBody(PrintWriter out) {
        if (autoRefresh) {
            out.println("<BODY text=\"#6C779F\" bgcolor=\"#FFFFFF\" link=\"#6C779F\" alink=\"#6C779F\" vlink=\"#6C779F\" onload=\"javascript:Refresh()\">\n");
        } else {
            out.println("<BODY text=\"#6C779F\" bgcolor=\"#FFFFFF\" link=\"#6C779F\" alink=\"#6C779F\" vlink=\"#6C779F\">\n");
        }
        out.println("<form name=\"refreshFormular\" action=\"http://" + serverIP + ":" + httpPort + "/" + "cidsservermanager\" method=\"get\"></form>\n" +
                ...
                "<br><font size=\"+1\" face=\"Courier New\"><b>" + serverType + " auf " + serverIP + "</b></font>\n" +
                ...
                "<tr>\n");
        if (autoRefresh) {
            out.println("<td valign=\"middle\"><font face=\"Courier New\" size=\"-1\">Der Server " + serverStatus + ".</font></td>\n");
        } else {
            out.println("<td valign=\"middle\"><font face=\"Courier New\" size=\"-1\" color=\"#606060\">Der Server " + serverStatus + ".</font></td>\n");
        }
        out.println("<td valign=\"middle\">\n" +
                "<a href=\"http://" + serverIP + ":" + httpPort + "/cidsservermanager?server=stop\">\n" +
                "<img src=\"/img/exit_space.png\" alt=\"Server beenden\" border=\"0\">&nbsp;Server beenden</a>&nbsp;&nbsp;&nbsp;\n" +
                "<a href=\"http://" + serverIP + ":" + httpPort + "/cidsservermanager?server=restart\">\n" +
                "<img src=\"/img/restart_space.png\" alt=\"Server neustarten\" border=\"0\">&nbsp;Server neustarten</a>&nbsp;&nbsp;&nbsp;\n" +
                //"<a href=\"http://" + serverIP + ":" + httpPort + "/cidsservermanager?server=kill\">\n" +
                //"<img src=\"/img/button_kill.gif\" alt=\"Kill\" border=\"0\"></a>&nbsp;&nbsp;&nbsp;\n" +
                "<a href=\"http://" + serverIP + ":" + httpPort + "/cidsservermanager?server=edit\">\n" +
                "<img src=\"/img/edit_space.png\" alt=\"Konfigurationsdatei editieren\" border=\"0\">&nbsp;Konfigurationsdatei editieren</a>&nbsp;&nbsp;&nbsp;\n" +
                
                "<a href=\"http://" + serverIP + ":" + httpPort + "/cidsservermanager?login=" + LOGOUT_PARAMETER_VALUE + "\">\n" +
                "<img src=\"/img/logout.png\" alt=\"Ausloggen\" border=\"0\">&nbsp;Ausloggen</a>&nbsp;&nbsp;&nbsp;\n" +
                "<a href=\"http://" + serverIP + ":" + httpPort + "/cidsservermanager?server=serverlog\">\n" +
     ...
     */
    private void printOutputTable(PrintWriter out) {
        out.println(
                "<tr>\n" +
                ...
                "<br><a href=\"http://" + serverIP + ":" + httpPort + "/cidsservermanager?server=refresh\">\n" +
                "<img src=\"/img/reload.png\" alt=\"Aktualisieren\" border=\"0\"><br>Aktualisieren</a><br>\n" +
                "<a href=\"http://" + serverIP + ":" + httpPort + "/cidsservermanager?server=clear\"><br>\n" +
                "<img src=\"/img/delete.png\" alt=\"Ausgabefenster l&ouml;schen\" border=\"0\"><br>Ausgabefenster l&ouml;schen</a><br>\n"
                );
        if (autoRefresh) {
            out.println("<a href=\"http://" + serverIP + ":" + httpPort + "/cidsservermanager?server=autoRefresh_off\"><br>\n" +
            ...
            out.println("<a href=\"http://" + serverIP + ":" + httpPort + "/cidsservermanager?server=autoRefresh_on\"><br>\n" +
        ...
        out.println("</td>\n" +
                ...
                " {document.write(\"<iframe src=\\\"/log/logOutput" + httpPort + ".htm#anchor\\\" width=\\\"825\\\" height=\\\"330\\\" name=\\\"logOutput\\\">\");}\n" +
                "else \n" +
                " {if (screen.width <= 1152)\n" +
                "  {document.write(\"<iframe src=\\\"/log/logOutput" + httpPort + ".htm#anchor\\\" width=\\\"920\\\" height=\\\"420\\\" name=\\\"logOutput\\\">\");}\n" +
                " else \n" +
                "  {document.write(\"<iframe src=\\\"/log/logOutput" + httpPort + ".htm#anchor\\\" width=\\\"1030\\\" height=\\\"515\\\" name=\\\"logOutput\\\">\");}\n" +
                "}" +
                "//-->\n" +
                "</script>\n" +
    ...
    private void printLogin(PrintWriter out) {
        printLogin(out, "");
    }
     ...
     */
    private void printLogin(PrintWriter out, String msg) {
...
//                + "</tr>\n"
    ...
    /**
     * Checks wether the user has already been authenticated.
     * Within the authentication process a special attribute of the
     * user's http session will be added. This allows to check if the
     * user has already entered a valid user/pass combo.
     *
     * @param request The user's request object
     * @return true, if the user has already been authenticated
     */
    private boolean isValidatedSession(HttpServletRequest request) {
        boolean ret = false;
        ...
        if ( request != null
            ...
            String validationAttribute = (String)request.getSession().getAttribute(VALIDATION_ATTRIBUTE);
    ...
    /**
     * Checks if a validation is in progress.
     * A validation is considered to be in progress if the corresponding action parameter
     * is found.
     * @return true if a validation is in progress
     */
    private boolean isValidationInProgress( HttpServletRequest request ) {
        boolean ret = false;
        
        
        String loginParamValue = request.getParameter( LOGIN_PARAMETER_NAME );
    ...
    /**
     * Performs the actual user validation by comparing the given usergroup,
     * username, password combination with the corresponding database.
     * @param request HTTP-Request containing the needed informations as
     *   request parameters.
     */
