...
public class SQLEditor extends JTextPane {
     ...
     * text.getSelectionStart();         int p1 = text.getSelectionEnd();         if (p0 != p1)         { try { Document
     * doc = text.getDocument();                 String srcData = doc.getText(p0, p1 - p0); StringSelection contents
     * =new StringSelection(srcData); clipboard.setContents(contents, null);            if (action ==
     * TransferHandler.MOVE)                 {               doc.remove(p0, p1 - p0);       }             } catch
     * (BadLocationException ble)   {}         }     } } public boolean importData(JComponent comp, Transferable t) { if
     * (comp instanceof JTextComponent)     {         DataFlavor flavor = getFlavor(t.getTransferDataFlavors());
     * System.out.println(t);                           if (flavor != null)  {             try             { String data
     * = "$" + (String)t.getTransferData(flavor) + "$"; ((JTextComponent)comp).replaceSelection(data);          return
     * true;             }              catch (UnsupportedFlavorException ufe)             { }              catch
     * (IOException ioe)             {    }         }     }     return false; }  public boolean canImport(JComponent
     * comp, DataFlavor[] transferFlavors) {     JTextComponent c = (JTextComponent)comp;     if (!(c.isEditable() &&
     * c.isEnabled()))     {      return false;     }     return (getFlavor(transferFlavors) != null); }  public int
